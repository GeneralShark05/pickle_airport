Config = {}

Config.Debug = true

Config.MissionCommand = true

Config.Blips = {}

Config.Tracker = {
    UpdateTime = 10,
    BlipTypes = {
        { -- Plane
            Label = "Plane",
            ID = 423,
            Color = 38,
            Scale = 0.75,
        },
        { -- Helicopter
            Label = "Helicopter",
            ID = 64,
            Color = 38,
            Scale = 0.75,
        },
    }
}

Config.Missions = {
    Blips = {
        PackagePickup = {
            Label = "Pickup Package",
            ID = 568,
            Color = 5,
            Scale = 0.75
        },
        PackageDropoff = {
            Label = "Drop-off Package",
            ID = 473,
            Color = 5,
            Scale = 0.75
        },
        PassengerPickup = {
            Label = "Pickup Passengers",
            ID = 685,
            Color = 5,
            Scale = 0.75
        },
        PassengerDropoff = {
            Label = "Drop-off Passengers",
            ID = 685,
            Color = 5,
            Scale = 0.75
        },
    },
    DistanceMultiplier = 0.10, -- * Distance between the two locations
    Rewards = {
        ['Passenger'] = { {name = "money", min = 500, max = 750} },
        ['Package'] = { {name = "money", min = 250, max = 350} }
    },
    payPerPassenger = true,
    passengerFare = {
        name = "money", min = 300, max = 500
    },
    payPerSize = true, -- Receives an extra cargo for: (Tonnage * (TopSpeed/200)) - Seats
    sizeFare = {
        name = "money", min = 300, max = 750
    }
}

Config.MissionTypes = {
    Passenger = {
        Model = `shamal`,
    },
    Delivery = {
        Model = `titan`,
    }
}

Config.AirportNames = { 'lsia', 'sandy', 'grapeseed'
}
Config.Airports = {
    ['lsia'] = {
        AirportTitle = "LSIA Airport",
        AirportDescription = "One of the busiest airports in the world, with nowhere near enough infrastructure to support it, Los Santos International brings third-world service to first-world travel.",
        Blips = {
            Boarding = {
                Label = "Airport",
                ID = 423,
                Color = 5,
                Scale = 0.75
            }
        },
        Locations = {
            Boarding = vector3(-1042.4271, -2745.5457, 21.3594),
            Flight = vector3(-1287.2458, -2599.4785, 14.5466),
            Hangar = vector4(-1058.8749, -2963.2598, 13.9645, 147.2194),
            ['Passenger'] = vector3(-1215.0138, -2647.5029, 14.5475),
            ['Delivery'] = vector3(-940.9973, -2954.2285, 13.9450)
        },
    },
    ['sandy'] = {
        AirportTitle = "Sandy Shores Airfield",
        AirportDescription = "A beach town nowhere near the ocean where the town is a trailer park and the beach is a mixture of toxic sludge and pulverized fish bones.",
        Blips = {
            Boarding = {
                Label = "Airport",
                ID = 423,
                Color = 5,
                Scale = 0.75
            }
        },
        Locations = {
            Boarding = vector3(1768.0939, 3296.6040, 41.1809),
            Flight = vector3(1740.0123, 3282.0764, 41.0884),
            Hangar = vector4(1766.9825, 3245.4485, 41.8806, 14.1263),
            ['Passenger'] = vector3(1713.7322, 3253.1628, 41.6815),
            ['Delivery'] = vector3(1720.2944, 3306.6436, 41.2235)
        },
    },
    ['grapeseed'] = {
        AirportTitle = "McKenzie Field",
        AirportDescription = "Located at the northeast tip of the Alamo Sea, Grapeseed is a thriving agricultural and industrial community involved in the manufacturing of high-fructose corn syrup and meth.",
        Blips = {
            Boarding = {
                Label = "Airport",
                ID = 423,
                Color = 5,
                Scale = 0.75
            }
        },
        Locations = {
            Boarding = vector3(2159.0381, 4782.0737, 41.9610),
            Flight = vector3(2140.8184, 4815.4282, 41.2161),
            Hangar = vector4(2149.6863, 4808.7354, 41.1853, 107.2027),
            ['Passenger'] = vector3(2124.8884, 4804.3428, 41.7431),
            ['Delivery'] = vector3(2122.36, 4784.67, 40.97)
        },
    },
}

Config.AirportSettings = {
    DepartureTime = 20,
    NPCFlightCost = 6500,
}

Config.Spawner = {
    Vehicles = {
        { label = "Luxor",        description = "", model = `luxor` },
        { label = "Shamal",       description = "", model = `shamal` },
        { label = "Cuban 800",    description = "", model = `cuban800` },
        { label = "Jet",          description = "", model = `jet` },
        { label = "Maverick",     description = "", model = `maverick` },
        { label = "Super Volito", description = "", model = `supervolito` },
    }
}

U = {}

U.permission_denied = "You can't do this."
U.permission_rewards = "You can't do this. You have not been paid."

-- Delivery Mission

U.package_pickup_notify = "Go to the cargo terminal."
U.package_pickup = "Press ~INPUT_CONTEXT~ to pick up the package."
U.package_load_notify = "Bring the package into the aircraft."
U.package_flight_notify = "Fly the aircraft to the desination."
U.package_dropoff_notify = "Go to the drop-off location."
U.package_dropoff = "Press ~INPUT_CONTEXT~ to drop-off package."

-- Passenger Mission

U.passenger_pickup_notify = "Go to the passenger terminal."
U.passenger_pickup = "Press ~INPUT_CONTEXT~ to pick up the passengers."
U.passenger_loading = "Loading Passengers... "
U.passenger_unloading = "Unloading Passengers..."
U.passenger_dropoff_notify = "Fly the aircraft to the desination. You have "
U.passenger_dropoff = "Press ~INPUT_CONTEXT~ to drop-off the passengers."

-- Airport
U.Boarding_interact = "Press ~INPUT_CONTEXT~ to board a flight."
U.Boarding_npc_interact = "Press ~INPUT_CONTEXT~ to fly for: $"
U.Boarding_unavailable = "There are no flights available."
U.Boarding_broke = "You can't afford to purchase this plane ticket."
U.Boarding_full = "You can't join the flight as it is full."

U.Flight_interact = "Press ~INPUT_CONTEXT~ to start a flight."
U.Flight_return = "Press ~INPUT_CONTEXT~ to drop-off your passengers."
U.Flight_reject = "You need to be in a plane to start a flight."
U.Flight_denied = "You can't start a flight."
U.Flight_needPlane = "You are not able to start a flight without being in a plane."

U.Hangar_interact = "Press ~INPUT_CONTEXT~ to retreive an aircraft."
U.Hangar_return = "Press ~INPUT_CONTEXT~ to return your aircraft."
U.Hangar_denied = "You can't access the hangar."
